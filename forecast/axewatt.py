...
class Axewatt:
    ...
    def get_model(self, region: str):
        model = load_model(f"./forecast/models/ONSModel_{region}", verbose=False)
    ...
    def create_models(self):
        for region in self.regions:
            try:
                ...
                model = timeseries_experiment.create_model("exp_smooth", verbose=False)

                tuned_model = timeseries_experiment.tune_model(
                    model, n_iter=10, verbose=False, search_algorithm="grid"
                )
